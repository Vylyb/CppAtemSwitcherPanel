...
#define	HTTP_ERROR_CONNECT_FAILED		3
#define	HTTP_ERROR_SEND_FAILED			4
#define	HTTP_ERROR_RECEIVE_FAILED		5
...
class VlcHttpConnection
{
public:
	VlcHttpConnection(CString ip, CString port);
	~VlcHttpConnection(void);
	bool	wasSuccessful();
	int		getHttpError();
...
private:
	bool	isPaused;
	CString	sendRequest(char* request);
	CString	sendStatusRequest(char* request);
	CString	sendStatusRequest(CString request);
	CString	sendRequest(CString request);
...
public:
	void setSelectedVideoID(CString id);
	void setSelectedVideoName(CString name);
};
